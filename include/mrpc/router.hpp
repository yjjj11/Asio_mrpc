...
struct function_traits<ReturnType(*)(Args...)> {
    ...
    static constexpr bool first_arg_is_conn = (total_argc >= 1) && std::is_same_v<first_arg_type, connection::cptr>;
...
class router {
  ...
  public:
    ...
    template<typename Function, typename SelfClass>
    void reg_handle(const std::string& name, Function f, std::shared_ptr<SelfClass> self, std::weak_ptr<asio::io_context> wio) {
        auto h = hash(name);
        invokes_[h] = { name, [f, self, wio](const std::shared_ptr<connection>& conn, const std::string& func_name, msg_id_t id, const std::string& buffer) {
			...
			io->dispatch([f, self, conn, func_name, id, buffer = std::move(buffer)]() {
                invoke_callback<Function, SelfClass>(f, self.get(), conn, func_name, id, buffer);
            });
            return true;
        }
                      };
    ...
    uint64_t query_msg_id(std::string_view key) {
        auto id = hash(key);
        std::shared_lock<std::shared_mutex> lock(msg_id_mutex_);
        if (msg_names_.find(id) == msg_names_.end()) {
            lock.unlock();
            std::unique_lock<std::shared_mutex> lock(msg_id_mutex_);
            if (msg_names_.find(id) == msg_names_.end()) {
  ...
  private:
    ...
    std::shared_mutex msg_id_mutex_;
