...
task_awaitable<RET> coro_call(const std::string& rpc_name, Args&&...args) {
    ...
    std::apply([&json_args](Args... args) {
        (json_args.emplace_back(std::forward<Args>(args)), ...);
    }, std::forward_as_tuple(args...));//依次解包到json_args数组中
  ...
  private:
    ...
    void on_callback_response(msg_id_t id) {
        ...
        {
            std::lock_guard<std::mutex> lock(cb_mtx_);
            auto iter = callback_map_.find(id.req_id);  // 修复：使用参数 id.req_id 而不是成员变量 req_id_
