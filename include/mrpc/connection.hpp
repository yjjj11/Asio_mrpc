...
class req_result {
  public:
    ...
    //如果调用函数的返回值是一个void类型，那么会将其 转换为int，否则暴捶==保持原类型
    req_result() = default;
...
class connection : public std::enable_shared_from_this<connection> {
  public:
    ...
    bool response(const nlohmann::json& ret_data) {
        ...
        //返回时发送的req_result格式的序列化数据
        //发送请求时发送的是msg_id_t  +   Args(参数列表)
        //那他们的解析逻辑是否一样呢
    }
    ...
    auto async_call(const std::string& rpc_name, Args&&...args) {
        ...
        //返回一个std::pair<msg_id_t,std::string>类型的future
        //前者用来指定具体请求的属性和请求id,后者包含error_code和error_msg，和ret_data
		...
		write(id, nlohmann::json::array_t{ std::forward<Args>(args)... });//写到对端服务器去
        //这里的args就是要调用的函数名和参数
    ...
    void  async_call(callback_t::callback_func_type cb, const std::string& rpc_name, Args&&...args) {
        ...
        uint64_t req_id = next_req_id();
        {
            ...
            });//如果超时之后响应还没有到来，也就是这个时钟还没有被设置为取消，那就从回调函数映射中移除
...
task_awaitable<RET> coro_call(const std::string& rpc_name, Args&&...args) {
    uint64_t req_id = next_req_id();
    // 将所有操作都放到挂起后执行
    auto suspend = [this, req_id, rpc_name, args_tuple = std::make_tuple(std::forward<Args>(args)...)]
    (stdcoro::coroutine_handle<> h) {
        auto msg_id = router().query_msg_id(rpc_name);
        uint32_t msg_type = ((1 << FMT) | (1 << MSG_IS_REQUEST) | (1 << MSG_IS_COROUTINE));
        msg_id_t id{ msg_type, msg_id, req_id };
        LOG_TRACE("coro_call suspend, {}({})", rpc_name, id);
        
        nlohmann::json::array_t json_args;
        std::apply([&json_args](auto&&... args) {
            (json_args.emplace_back(std::forward<decltype(args)>(args)), ...);
        }, args_tuple);//依次解包到json_args数组中
    ...
    auto resume = [this, req_id]() -> req_result<RET> {
        return req_result<RET>({this->msg_type_, this->msg_id_, this->req_id_}, this->msg_body_);
    };

    // coro_call主流程：仅创建等待器并返回，无任何业务逻辑，耗时趋近于零
    return task_awaitable<RET>(std::move(suspend), std::move(resume));
}
#endif
  ...
  private:
    ...
    void do_read_body() {
        ...
        msg_body_.resize(msg_len_);//只负责读入消息体的内容，解析细分到按照 消息类型来分类的处理函数中
  ...
  private:
    ...
    void on_callback_response(msg_id_t id) {
        ...
        auto json = router::decode(id.msg_type, msg_body_);//按照id的msg_type来解码
	...
	void write() {//单线程写入同一个发送队列然后再异步发送，保证pop和emplac_back的线程安全要加锁
		if (!socket_.is_open()) {
			return;
		}
...
bool mrpc::router::invoke_callback(Function f, SelfClass* self,
    ...
    //萃取函数特征
    using FuncTraits = function_traits<Function>;
    ...
    try {
        // 补全缺失的业务参数（兼容参数个数不足的情况）
        if (json.size() < FuncTraits::argc) {
            LOG_WARN("msg: {} parameter not full, need: {}, got: {}", id, FuncTraits::argc, json.size());
            parameter_extend(json, args, std::make_index_sequence<FuncTraits::argc> {});
        }
        // 将json格式的参数转换为业务参数元组
        ...
        if constexpr (std::is_null_pointer_v<SelfClass>) {
            if constexpr (need_pass_conn) {
                // 带conn：conn + 业务参数 拼接调用
                if constexpr (std::is_void_v<typename FuncTraits::return_type>) {
                    std::apply(f, std::tuple_cat(std::make_tuple(conn), std::move(args)));
                } else {
                    jret = std::apply(f, std::tuple_cat(std::make_tuple(conn), std::move(args)));
                }
            } else {
                // 无conn：直接调用业务参数（兼容无参函数）
                if constexpr (std::is_void_v<typename FuncTraits::return_type>) {
                    if constexpr (FuncTraits::argc == 0) f(); // 无参无返回
                    else std::apply(f, std::move(args));      // 有参无返回
                } else {
                    if constexpr (FuncTraits::argc == 0) jret = f(); // 无参有返回
                    else jret = std::apply(f, std::move(args));      // 有参有返回
                }
            }
        } else {
            // 类成员函数：根据need_pass_conn判断是否拼接conn（先拼self指针）
            if constexpr (need_pass_conn) {
                // 带conn：self + conn + 业务参数 拼接调用
                if constexpr (std::is_void_v<typename FuncTraits::return_type>) {
                    std::apply(f, std::tuple_cat(std::make_tuple(self, conn), std::move(args)));
                } else {
                    jret = std::apply(f, std::tuple_cat(std::make_tuple(self, conn), std::move(args)));
                }
            } else {
                // 无conn：self + 业务参数 拼接调用（兼容无参成员函数）
                if constexpr (std::is_void_v<typename FuncTraits::return_type>) {
                    if constexpr (FuncTraits::argc == 0) (self->*f)(); // 无参无返回
                    else std::apply(f, std::tuple_cat(std::make_tuple(self), std::move(args))); // 有参无返回
                } else {
                    if constexpr (FuncTraits::argc == 0) jret = (self->*f)(); // 无参有返回
                    else jret = std::apply(f, std::tuple_cat(std::make_tuple(self), std::move(args))); // 有参有返回
                }
            }
        }
    } catch (const nlohmann::json::exception& e) {
        // 单独捕获JSON解析异常，日志更精准
        LOG_ERROR("json parse error: {}, content: {}", e.what(), json.dump());
        return !is_no_response ? conn->response(id, status::bad_request, "param parse error", nullptr) : true;
    } catch (const std::exception& e) {
        // 捕获其他运行时异常
        LOG_ERROR("function invoke error: {}", e.what());
        return !is_no_response ? conn->response(id, status::internal_server_error, "server inner error", nullptr) : true;
    } catch (...) {
        // 捕获未知异常，避免服务崩溃
        LOG_ERROR("function invoke unknown error");
...
#endif // MRPC_CONNECTION_HPPC_CONNECTION_HPP